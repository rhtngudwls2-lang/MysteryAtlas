...
export default async function CasePage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  return <article className="case-page">
    <header className="case-hero">
      <div className="case-hero-copy"><div className="eyebrow"><span>{record.country[locale]}</span><span>{record.year}</span><span>{record.status[locale]}</span></div><h1>{record.title}</h1><p className="case-subtitle">{record.subtitle[locale]}</p><p className="case-preview">{record.preview[locale]}</p><div className="case-actions"><SaveButton slug={record.slug} locale={locale}/><ShareButton locale={locale} title={record.title}/></div><div className="verification-line"><span>{readingMinutes(record, locale)} {locale === "en" ? "min read" : "분 읽기"}</span><span>{locale === "en" ? "Last verified" : "마지막 검증"}: {record.verifiedAt}</span></div></div>
    ...
    <div className="article-layout">
      ...
      <div className="article-main">
        ...
        <section id="rabbit-hole" className="rabbit-section"><p className="section-number">04 / RABBIT HOLE</p><h2>{locale === "en" ? "Follow the question, not just the category" : "카테고리가 아니라 질문을 따라가세요"}</h2><div>{record.related.map((related) => { const target = caseBySlug(related.slug); return target ? <Link key={related.slug} href={`/${locale}/cases/${related.slug}/`}><span>{related.question[locale]}</span><strong>{target.title} →</strong></Link> : null; })}</div></section>
      </div>
    </div>
  </article>;
}
