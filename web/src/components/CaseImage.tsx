...
export function CaseImage({ image, locale, priority = false }: { image: ImageRecord; locale: Locale; priority?: boolean }) {
  if (!image.path) return <figure className="image-placeholder" aria-label={image.alt[locale]}>
    ...
    <figcaption><strong>{locale === "en" ? "Verified image unavailable" : "검증된 이미지 없음"}</strong><span>{image.caption[locale]}</span></figcaption>
  </figure>;
  return <figure className="case-image">
    ...
    <figcaption><span className="image-type">{image.type.replaceAll("_", " ")}</span>{image.caption[locale]}</figcaption>
  </figure>;
}
