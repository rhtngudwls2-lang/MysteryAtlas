...
export function SearchView({ locale }: { locale: Locale }) {
  ...
  return <div className="search-view">
    <label className="search-box"><span className="sr-only">{locale === "en" ? "Search cases" : "사건 검색"}</span><input autoComplete="off" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={locale === "en" ? "Search a case, person, place, date…" : "사건, 인물, 장소, 날짜 검색…"} data-testid="search-input"/><kbd>⌘ K</kbd></label>
