...
@Composable private fun RabbitScreen(data:Catalog,lang:String,vm:AtlasViewModel){val path by vm.path.collectAsStateWithLifecycle();val user by vm.user.collectAsStateWithLifecycle();val current=data.cases.firstOrNull{it.id==path.lastOrNull()}?:data.cases.first();LazyColumn(Modifier.fillMaxSize().testTag("rabbit_list"),contentPadding=PaddingValues(22.dp)){
 ...
 item{Surface(color=Panel,shape=RoundedCornerShape(12.dp),border=BorderStroke(1.dp,Line)){Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(10.dp)){Text(tr(lang,"현재 연결 지점","CURRENT CONNECTION"),color=Muted,style=MaterialTheme.typography.labelSmall);StoryMeta(current,lang);Text(current.headline.text(lang),style=MaterialTheme.typography.titleLarge);Text(current.hook.text(lang),color=Muted,style=MaterialTheme.typography.bodyMedium);ReadMeta(current,lang);Art(current.image,lang,Modifier.fillMaxWidth().height(150.dp),sharedImageNote(current,lang));sharedImageNote(current,lang)?.let{Text(it,color=Muted,style=MaterialTheme.typography.labelSmall)}}}}
 item{Column(Modifier.testTag("rabbit_path")){path.forEachIndexed{index,id->val visited=data.cases.firstOrNull{it.id==id};if(visited!=null)TextButton(onClick={vm.revisit(id)},modifier=Modifier.fillMaxWidth().testTag("path_$id")){Text("${index+1}. ${visited.headline.text(lang)}",modifier=Modifier.fillMaxWidth())}};SectionTitle(tr(lang,"다음 의문을 선택하세요","Choose your next question"),tr(lang,"공통 주제로 연결한 이야기입니다. 인과관계를 뜻하지 않습니다.","Connected by theme, not by a claim of causation."))}}
